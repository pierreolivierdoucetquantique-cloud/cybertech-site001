...
def generate_invoice_pdf(
    ...
    line_items : liste de dicts {"name": str, "description": str, "price": float},
    un par service du panier (v9.11 — remplace l'ancien product_name unique).

    status / deposit_amount / remaining_balance / amount_paid (v9.12) :
    pilotent l'affichage automatique du bloc "Paiement" :
      - status == "Deposit Paid" : affiche Total du projet / Dépôt payé (40%) /
        Solde restant (60%). Le dépôt N'EST JAMAIS affiché en dehors de ce cas.
      - status == "Paid in Full" (ou tout autre statut) : affiche uniquement
        Total / Montant payé / Solde = 0,00 $ / Statut "Payé intégralement".
    """
    buffer = io.BytesIO()
    ...
    for item in line_items:
        data.append([
            Paragraph(f"<b>{item['name']}</b><br/>{item.get('description', '')}", table_cell_style),
            Paragraph(f"{item['price']:,.2f} $", ParagraphStyle("amt", parent=table_cell_style, alignment=TA_RIGHT)),
        ])
    ...
    product_table.setStyle(TableStyle(row_styles))
    elements.append(product_table)
    elements.append(Spacer(1, 14))

    # --- Totaux (les lignes TPS/TVQ ne s'affichent que si applicables) ---
    totals_data = [["Sous-total", f"{subtotal:,.2f} $"]]
    if gst_amount > 0 or qst_amount > 0:
        totals_data.append(["TPS (5%)", f"{gst_amount:,.2f} $"])
        totals_data.append(["TVQ (9.975%)", f"{qst_amount:,.2f} $"])
    ...
    # --- Bloc Paiement (v9.12) : dépôt/solde OU payé intégralement, jamais les deux ---
    is_deposit_case = status == "Deposit Paid" and deposit_amount is not None
    payment_rows = []
    if is_deposit_case:
        payment_rows = [
            ["Total du projet", f"{total:,.2f} $"],
            ["Dépôt payé (40%)", f"{deposit_amount:,.2f} $"],
            ["Solde restant (60%)", f"{(remaining_balance or 0):,.2f} $"],
            ["Statut", "Dépôt payé"],
        ]
        ...
        status_labels_fr = {
            ...
            "Refunded": "Remboursé",
            "Partially Refunded": "Remboursé partiellement",
        }
        paid = amount_paid if amount_paid is not None else total
        payment_rows = [
            ["Montant total", f"{total:,.2f} $"],
            ["Montant payé", f"{paid:,.2f} $"],
            ["Solde", f"{(remaining_balance or 0):,.2f} $"],
            ["Statut", status_labels_fr.get(status, status)],
        ]

    if payment_rows:
        payment_table = Table(payment_rows, colWidths=[220, 90])
        payment_table.setStyle(TableStyle([
            ("FONTSIZE", (0, 0), (-1, -1), 9.5),
            ("TEXTCOLOR", (0, 0), (0, -1), SOFT_COLOR),
        ...
        payment_wrapper = Table([[payment_table]], colWidths=[None])
        payment_wrapper.setStyle(TableStyle([("ALIGN", (0, 0), (0, 0), "RIGHT")]))
        elements.append(Paragraph("<font color='#0F172A' size=10><b>Paiement</b></font>", style_value))
