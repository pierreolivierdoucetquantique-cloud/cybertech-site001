...
def send_order_confirmation_email(client: "Client", order: "Order") -> bool:
    is_maintenance = getattr(order.product_type, "value", order.product_type) == "maintenance"
    next_step_html = (
        "<p>Prochaine étape : veuillez signer le <strong>contrat de maintenance</strong> "
        "depuis votre espace client afin de finaliser votre commande.</p>"
        if is_maintenance else
        "<p>Prochaine étape : veuillez remplir le <strong>questionnaire technique</strong> "
        "depuis votre espace client afin que nous puissions démarrer votre projet.</p>"
    ...
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;">
      ...
      <table style="width:100%;border-collapse:collapse;margin:16px 0;">
        <tr><td style="padding:6px 0;color:#666;">N° de commande</td><td style="padding:6px 0;text-align:right;font-weight:bold;">{order.order_number}</td></tr>
        <tr><td style="padding:6px 0;color:#666;">Service</td><td style="padding:6px 0;text-align:right;">{order.product_name}</td></tr>
        <tr><td style="padding:6px 0;color:#666;">Montant</td><td style="padding:6px 0;text-align:right;">{order.price:,.2f} $ CAD</td></tr>
      </table>
      {next_step_html}
...
def _notify_admin_new_order(client: "Client", order: "Order") -> bool:
    ...
    <div style="font-family:Arial,sans-serif;">
      ...
      <p><strong>Commande :</strong> {order.order_number} — {order.product_name} — {order.price:,.2f} $ CAD</p>
    </div>
    """
    return _send(settings.ADMIN_NOTIFICATION_EMAIL, f"Nouvelle commande : {order.order_number}", html)


def send_technical_form_email(client: "Client", order: "Order", form_summary_html: str) -> bool:
    html = f"""
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;">
      <h2 style="color:#2563EB;">Questionnaire technique reçu</h2>
      <p>Bonjour {client.first_name},</p>
      <p>Merci d'avoir complété le questionnaire technique pour votre commande
      <strong>{order.order_number}</strong>. Notre équipe va l'analyser et vous
    ...
    admin_html = f"""
    <div style="font-family:Arial,sans-serif;">
      <h3>Formulaire technique soumis — {order.order_number}</h3>
      <p><strong>Client :</strong> {client.first_name} {client.last_name} ({client.email})</p>
      {form_summary_html}
    </div>
    """
    _send(settings.ADMIN_NOTIFICATION_EMAIL, f"Formulaire technique — {order.order_number}", admin_html)
