...
def global_search(
    ...
    # v9.11 : recherche aussi par nom de service (OrderItem.product_name),
    # puisque ce champ n'est plus directement sur Order.
    order_ids_matching_items = (
        db.query(OrderItem.order_id)
        .filter(OrderItem.product_name.ilike(like))
        .subquery()
    )
    orders = (
        db.query(Order)
        ...
        .filter(or_(
            ...
            Order.id.in_(order_ids_matching_items),
    ...
    for o in orders:
        items_label = ", ".join(i.product_name for i in o.items)
        results.append({
            "type": "order",
            "id": o.id,
            "title": o.order_number,
            "subtitle": f"{o.client.first_name} {o.client.last_name} — {items_label}",
            "panel": "orders",
        })
