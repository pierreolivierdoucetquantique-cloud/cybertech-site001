...
/* ---------- Sous-onglets "Mes commandes" (Historique / Votre panier) ---------- */
function setupOrdersSubtabs() {
  const subtabs = document.querySelectorAll('.orders-subtab');
  const subpanels = document.querySelectorAll('.orders-subpanel');
  subtabs.forEach(subtab => {
    subtab.addEventListener('click', () => {
      ...
      document.querySelector(`.orders-subpanel[data-subpanel="${subtab.dataset.subpanel}"]`)?.classList.add('is-active');
    });
  });
}
