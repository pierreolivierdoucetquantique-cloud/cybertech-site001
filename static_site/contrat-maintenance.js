...
// v9.11 : un contrat par OrderItem (service Maintenance), pas par commande entière.
// ===========================================================

let mcItemId = null;
let mcOrderNumber = null;
let mcContract = null;
let mcSigMode = 'drawn';
let mcSigCtx = null;
...
async function initMaintenanceContract() {
  ...
  mcItemId = params.get('item');
  const legacyOrderId = params.get('order');

  if (!mcItemId && !legacyOrderId) {
    guard.hidden = false;
  ...
  try {
    ...
    if (legacyOrderId && !mcItemId) {
      ...
      for (const o of myOrders) {
        const found = o.items.find(i => i.id === mcItemId);
    ...
    guardText.textContent = 'Vous devez être connecté pour accéder à cette commande, ou ce service est introuvable.';
    return;
  }

  if (item.product_type !== 'maintenance') {
    guard.hidden = false;
    guardTitle.textContent = 'Service non concerné';
    guardText.textContent = 'Ce service n\'est pas un contrat de maintenance.';
    return;
  }

  mcOrderNumber = order.order_number;
  document.getElementById('mc-order-label').textContent = `${order.order_number} — ${item.product_name}`;
  ...
  try {
    mcContract = await Api.getMaintenanceContract(mcItemId);
    ...
    renderContractDetailsDefaults(item);
  }
}

function renderContractDetailsDefaults(item) {
  document.getElementById('mc-plan').textContent = item.product_name;
  document.getElementById('mc-price').textContent = `${item.price.toLocaleString('fr-CA')} $ CAD / an`;
...
function setupInfoForm(profile) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      mcContract = await Api.saveMaintenanceContractInfo(mcItemId, payload);
      renderContractDetails(mcContract);
...
async function handleSign() {
  ...
  try {
    mcContract = await Api.signMaintenanceContract(mcItemId, {
...
function showSignedState(contract) {
  ...
  link.href = `/api/order-items/${mcItemId}/maintenance-contract/download`;
  banner.hidden = false;
}
