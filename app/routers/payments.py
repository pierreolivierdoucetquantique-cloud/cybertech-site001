"""Router : paiements côté client (Interac et Stripe)."""
...
from app.services.payment_gate import assert_payment_unlocked
...
def create_interac_payment(
    ...
    amount = calculate_payment_amount(order, payload.amount_type)
    payment = Payment(
        order_id=order.id,
        client_id=client.id,
