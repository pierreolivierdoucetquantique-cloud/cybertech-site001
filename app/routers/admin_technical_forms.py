"""Router : consultation des questionnaires techniques par l'admin."""
...
def _form_to_dict(form: TechnicalForm, order: Optional[Order], client: Optional[Client]) -> dict:
    return {
        "id": form.id,
...
def list_technical_forms(
    ...
    results = []
    for form in forms:
        order = db.query(Order).filter(Order.id == form.order_id).first()
        client = db.query(Client).filter(Client.id == form.client_id).first()
        results.append((form, order, client))

    if search:
        s = search.strip().lower()
        results = [
            (f, o, c) for f, o, c in results
    ...
    return [_form_to_dict(f, o, c) for f, o, c in results]
...
def get_technical_form_detail(
    ...
    order = db.query(Order).filter(Order.id == form.order_id).first()
    client = db.query(Client).filter(Client.id == form.client_id).first()
    return _form_to_dict(form, order, client)
