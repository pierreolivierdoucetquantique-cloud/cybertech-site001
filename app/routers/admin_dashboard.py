...
from app.models.order import Order, OrderItem, OrderStatus, ProductType, PaymentStatus
...
def get_dashboard_stats(
    ...
    # v9.12 : "revenu total" reflète maintenant l'argent RÉELLEMENT encaissé
    # (paiements approuvés, nets des remboursements) — pas le total facturé,
    # qui peut inclure des soldes de 60% pas encore payés.
    total_revenue = get_total_collected_revenue(db)
    ...
    # v9.11 : "maintenance" est désormais un OrderItem (un service du panier),
    # pas l'Order entière. On compte les OrderItem MAINTENANCE dont le panier
    # parent est payé et non annulé.
    active_maintenance = (
        db.query(func.count(OrderItem.id))
        .join(Order, OrderItem.order_id == Order.id)
        .filter(
            OrderItem.product_type == ProductType.MAINTENANCE,
