...
logger = logging.getLogger("ctq.main")
...
from app.routers import (
    ...
    admin_search, maintenance_contract, admin_settings,
)
...
async def global_exception_handler(request: Request, exc: Exception):
    """
    Filet de sécurité : toute exception non gérée (Stripe, DB, etc.) est
    loguée avec sa trace complète côté serveur (visible dans les logs Render)
    et renvoyée au frontend en JSON avec un message générique, au lieu de la
    page texte brute par défaut de Starlette (que le frontend ne peut pas
    parser, d'où le message '(500)' sans détail).
    """
    logger.exception("Erreur non gérée sur %s %s", request.method, request.url.path)
    return JSONResponse(
        status_code=500,
        content={"detail": "Une erreur interne est survenue. Veuillez réessayer ou contacter le support si le problème persiste."},
    )
