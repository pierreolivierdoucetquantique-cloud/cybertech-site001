...
const INVOICE_STATUS_LABELS = {
  'Deposit Paid': 'Dépôt payé', 'Paid in Full': 'Payé intégralement',
  'Refunded': 'Remboursé', 'Partially Refunded': 'Remboursé partiellement',
};
...
async function loadStats() {
  ...
  try {
    ...
    grid.innerHTML = `
      ...
      <div class="admin-stat-card accent"><div class="admin-stat-value">${s.total_revenue.toLocaleString('fr-CA')} $</div><div class="admin-stat-label">Revenu encaissé (taxes incl.)</div></div>
...
async function openClientProfileModal(clientId) {
  ...
  try {
    ...
    const ordersRows = data.orders.length > 0
      ? data.orders.map(o => {
          const servicesLabel = o.items.map(i => `${escapeHtml(i.product_name)} (${i.project_progress}%)`).join(', ');
          return `
          <tr>
            <td>${escapeHtml(o.order_number)}</td>
            <td style="max-width:220px; white-space:normal;">${servicesLabel}</td>
            <td>${o.total.toLocaleString('fr-CA')} $</td>
      ...
      : `<tr><td colspan="5" class="admin-empty">Aucune commande.</td></tr>`;
    ...
    content.innerHTML = `
      <div class="admin-stats-grid" style="grid-template-columns:repeat(3, 1fr); margin-bottom:20px;">
        ...
        <div class="admin-stat-card accent"><div class="admin-stat-value">${data.total_revenue.toLocaleString('fr-CA')} $</div><div class="admin-stat-label">Revenu encaissé</div></div>
      </div>
      ...
      <div class="admin-table-wrap" style="margin-bottom:18px;">
        <table class="admin-table">
          <thead><tr><th>N°</th><th>Services</th><th>Total</th><th>Statut</th><th>Paiement</th></tr></thead>
...
function updateProgressPreview(progress, valueEl, stepEl, fillEl) {
  valueEl.textContent = progress;
  stepEl.textContent = stepLabelFor(progress);
  fillEl.style.width = `${progress}%`;
  fillEl.style.background = { red: '#DC2626', yellow: '#F59E0B', green: '#16A34A' }[colorFor(progress)];
}
...
async function loadOrders() {
  ...
  try {
    ...
    tbody.innerHTML = allOrders.map(o => {
      const servicesLabel = o.items.map(i => i.product_name).join(', ');
      const avgProgress = o.items.length
        ? Math.round(o.items.reduce((sum, i) => sum + i.project_progress, 0) / o.items.length)
        : 0;
      return `
      <tr>
        <td>${escapeHtml(o.order_number)}</td>
        <td>${escapeHtml(o.client_name)}<br><span style="color:var(--text-soft);font-size:.78rem;">${escapeHtml(o.client_email)}</span></td>
        <td>${escapeHtml(servicesLabel)} ${o.items.length > 1 ? `<span class="admin-badge">${o.items.length} services</span>` : ''}</td>
        <td>${o.total.toLocaleString('fr-CA')} $</td>
...
function openOrderModal(orderId) {
  ...
  document.getElementById('order-modal-number').textContent = `— ${order.order_number}`;

  const listContainer = document.getElementById('order-items-progress-list');
  const template = document.getElementById('order-item-progress-template');
  listContainer.innerHTML = '';

  order.items.forEach(item => {
    const block = template.content.cloneNode(true);
    const root = block.querySelector('.order-item-progress-block');
    block.querySelector('.oip-name').textContent = item.product_name;
    block.querySelector('.oip-price').textContent = `${item.price.toLocaleString('fr-CA')} $`;

    const slider = block.querySelector('.oip-progress-slider');
    const valueEl = block.querySelector('.oip-progress-value');
    const stepEl = block.querySelector('.oip-progress-step');
    const fillEl = block.querySelector('.oip-progress-preview');
    const dateInput = block.querySelector('.oip-delivery-date');
    const saveBtn = block.querySelector('.oip-save-btn');

    slider.value = item.project_progress;
    dateInput.value = item.expected_delivery_date || '';
    updateProgressPreview(item.project_progress, valueEl, stepEl, fillEl);

    slider.addEventListener('input', () => updateProgressPreview(parseInt(slider.value, 10), valueEl, stepEl, fillEl));
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        await Api.adminUpdateOrderItem(order.id, item.id, {
          project_progress: parseInt(slider.value, 10),
          expected_delivery_date: dateInput.value || null,
        });
        await loadOrders();
        saveBtn.textContent = 'Enregistré ✓';
        setTimeout(() => { saveBtn.textContent = restoreLabel; saveBtn.disabled = false; }, 1200);
      } catch (err) {
        alert(err.message);
...
async function loadInvoices() {
  ...
  try {
    ...
    if (allInvoices.length === 0) {
      tbody.innerHTML = `<tr><td colspan="10" class="admin-empty">Aucune facture trouvée.</td></tr>`;
      return;
    }
    tbody.innerHTML = allInvoices.map(inv => {
      const statusLabel = INVOICE_STATUS_LABELS[inv.status] || inv.status || '—';
      const depositLine = (inv.status === 'Deposit Paid' && inv.deposit_amount != null)
        ? `<br><span style="color:var(--admin-text-soft);font-size:.78rem;">Dépôt : ${inv.deposit_amount.toLocaleString('fr-CA')} $ · Solde : ${(inv.remaining_balance || 0).toLocaleString('fr-CA')} $</span>`
        : '';
      return `
      <tr>
        <td>${escapeHtml(inv.invoice_number)}</td>
        ...
        <td><span class="status-badge">${statusLabel}</span>${depositLine}</td>
        <td>${new Date(inv.created_at).toLocaleDateString('fr-CA')}</td>
        <td class="admin-actions-cell">
    ...
    tbody.innerHTML = `<tr><td colspan="10" class="admin-empty">Erreur de chargement.</td></tr>`;
  }
}
...
/* ---------- Paiements (v9.11 : historique complet + remboursement) ---------- */
let allPayments = [];
...
function setupPaymentsPanel() {
  ...
  document.getElementById('payments-method-filter').addEventListener('change', loadPayments);
  document.getElementById('payments-sort').addEventListener('change', loadPayments);
  document.getElementById('payments-search').addEventListener('input', debounce(loadPayments, 350));
  document.getElementById('export-payments-btn').addEventListener('click', () => Api.adminExportPaymentsCsv());
  ...
  document.getElementById('cancel-payment-refund').addEventListener('click', () => {
    document.getElementById('payment-refund-modal').hidden = true;
  });
  document.getElementById('payment-refund-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const form = e.target;
    const amountRaw = form.querySelector('[name="amount"]').value;
    const reason = form.querySelector('[name="reason"]').value || null;
    const amount = amountRaw ? parseFloat(amountRaw) : null;
    const submitBtn = form.querySelector('button[type="submit"]');
    const restore = setButtonLoading(submitBtn, 'Remboursement…');
    try {
      await Api.adminRefundPayment(currentRefundPaymentId, amount, reason);
      document.getElementById('payment-refund-modal').hidden = true;
      showAdminToast('Remboursement effectué.', 'success');
...
const PAYMENT_METHOD_LABELS_ADMIN = { interac: 'Interac', stripe: 'Stripe (carte)' };

function buildPaymentsQuery() {
  const pendingOnly = document.getElementById('payments-filter').value;
  const method = document.getElementById('payments-method-filter').value;
  const search = document.getElementById('payments-search').value.trim();
  const [sortBy, sortDir] = document.getElementById('payments-sort').value.split('-');

  const parts = [];
  if (pendingOnly === 'true') parts.push('pending_only=true');
...
async function loadPayments() {
  ...
  try {
    allPayments = await Api.adminListPayments(buildPaymentsQuery());
    if (allPayments.length === 0) {
      tbody.innerHTML = `<tr><td colspan="12" class="admin-empty">Aucun paiement trouvé.</td></tr>`;
      return;
    }
    tbody.innerHTML = allPayments.map(p => {
      const cardInfo = p.card_brand ? `<br><span style="color:var(--text-soft);font-size:.76rem;">${escapeHtml(p.card_brand)} ••${escapeHtml(p.card_last4 || '')}</span>` : '';
      const refundInfo = p.refunded_amount ? `<br><span style="color:var(--text-soft);font-size:.76rem;">Remboursé : ${p.refunded_amount.toLocaleString('fr-CA')} $</span>` : '';
      const canReview = p.status === 'pending_validation';
      const canRefund = p.status === 'approved' && p.method === 'stripe';
      return `
      <tr>
        <td>${new Date(p.created_at).toLocaleDateString('fr-CA')}<br><span style="color:var(--text-soft);font-size:.76rem;">${new Date(p.created_at).toLocaleTimeString('fr-CA', { hour: '2-digit', minute: '2-digit' })}</span></td>
        ...
        <td style="max-width:180px; white-space:normal; font-size:.82rem;">${escapeHtml(p.items_summary || '—')}</td>
        <td>${p.order_subtotal != null ? p.order_subtotal.toLocaleString('fr-CA') + ' $' : '—'}</td>
        <td>${p.order_gst_amount != null ? p.order_gst_amount.toLocaleString('fr-CA') + ' $' : '—'}</td>
        <td>${p.order_qst_amount != null ? p.order_qst_amount.toLocaleString('fr-CA') + ' $' : '—'}</td>
        <td><strong>${p.order_total != null ? p.order_total.toLocaleString('fr-CA') + ' $' : '—'}</strong></td>
        <td>${PAYMENT_METHOD_LABELS_ADMIN[p.method] || p.method}${cardInfo}</td>
        <td>${p.amount.toLocaleString('fr-CA')} $<br><span style="color:var(--text-soft);font-size:.76rem;">${p.amount_type === 'deposit' ? 'Dépôt (40%)' : 'Total'}</span></td>
        <td><span class="admin-badge payment-status-${p.status}">${PAYMENT_REQUEST_LABELS[p.status] || p.status}</span>${refundInfo}</td>
        <td class="admin-actions-cell">
          ${canReview ? `<button class="admin-btn-sm primary" data-action="review-payment" data-id="${p.id}">Valider</button>` : ''}
          ${canRefund ? `<button class="admin-btn-sm danger" data-action="refund-payment" data-id="${p.id}">Rembourser</button>` : ''}
          ${p.proof_file_id ? `<a href="/api/files/${p.proof_file_id}" target="_blank" rel="noopener" class="admin-btn-sm">Preuve</a>` : ''}
          ${!canReview && !canRefund && !p.proof_file_id ? '—' : ''}
        </td>
      </tr>
    `;
    }).join('');
    ...
    tbody.querySelectorAll('[data-action="refund-payment"]').forEach(btn => {
      btn.addEventListener('click', () => openPaymentRefundModal(btn.dataset.id));
    });
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="12" class="admin-empty">Erreur de chargement.</td></tr>`;
  }
}
...
const PAYMENT_REQUEST_LABELS = {
  ...
  refunded: 'Remboursé', cancelled: 'Annulé',
};
...
function openPaymentReviewModal(paymentId) {
  ...
  document.getElementById('payment-review-content').innerHTML = `
    ...
    <p>Commande : <strong>${escapeHtml(p.order_number || '')}</strong> — ${escapeHtml(p.items_summary || '')}</p>
...
function openPaymentRefundModal(paymentId) {
  const p = allPayments.find(x => x.id === paymentId);
  if (!p) return;
  currentRefundPaymentId = paymentId;
  document.getElementById('payment-refund-content').innerHTML = `
    <p><strong>${escapeHtml(p.client_name || '')}</strong> — ${escapeHtml(p.client_email || '')}</p>
    <p>Commande : <strong>${escapeHtml(p.order_number || '')}</strong></p>
    <p>Montant payé : <strong>${p.amount.toLocaleString('fr-CA')} $ CAD</strong> via ${PAYMENT_METHOD_LABELS_ADMIN[p.method] || p.method}</p>
  `;
  const form = document.getElementById('payment-refund-form');
  form.reset();
  form.querySelector('[name="amount"]').max = p.amount;
  document.getElementById('payment-refund-modal').hidden = false;
}
...
function setupSettingsPanel() {
  ...
  // Réglage TPS/TVQ (v9.11)
  const taxesCheckbox = document.getElementById('settings-enable-taxes');
  Api.adminGetTaxesSetting()
    .then(s => { taxesCheckbox.checked = s.enable_tps_tvq; })
    .catch(() => { /* silencieux : on garde la case décochée par défaut en cas d'erreur réseau */ });

  taxesCheckbox.addEventListener('change', async () => {
    const newValue = taxesCheckbox.checked;
    taxesCheckbox.disabled = true;
    try {
      await Api.adminUpdateTaxesSetting(newValue);
      showAdminToast(newValue ? 'TPS/TVQ activées pour les nouvelles commandes.' : 'TPS/TVQ désactivées pour les nouvelles commandes.', 'success');
    } catch (err) {
      taxesCheckbox.checked = !newValue; // annule le changement visuel si l'appel échoue
      showAdminToast(err.message || 'Une erreur est survenue.', 'error');
    } finally {
      taxesCheckbox.disabled = false;
    }
  });
}
