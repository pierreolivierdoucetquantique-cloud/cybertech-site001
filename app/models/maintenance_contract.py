...
class MaintenanceContract(Base):
    ...
    # v9.11 : lié à un OrderItem (un service Maintenance précis du panier), plus à Order
    ...
    # --- Rappel de renouvellement (v9.11) ---
    # Évite d'envoyer le rappel "expire dans 30 jours" plusieurs fois ; vérifié
    # à chaque connexion admin (pas de scheduler en arrière-plan sur Render free tier).
    renewal_reminder_sent_at = Column(DateTime, nullable=True)

    created_at = Column(DateTime, default=dt.datetime.utcnow)
    updated_at = Column(DateTime, default=dt.datetime.utcnow, onupdate=dt.datetime.utcnow)

    order_item = relationship("OrderItem", back_populates="maintenance_contract")
