...
async function loadInvoices() {
  ...
  try {
    ...
    list.innerHTML = invoices.map(inv => {
      ...
      const invoiceStatusLabel = INVOICE_STATUS_LABELS[inv.invoice_status] || PAYMENT_STATUS_LABELS[inv.payment_status] || inv.payment_status || '—';
      ...
      const isDepositCase = inv.invoice_status === 'Deposit Paid' && inv.deposit_amount != null;
      const paymentBreakdown = isDepositCase
        ? `<tr><td>Dépôt payé (40%)</td><td>${inv.deposit_amount.toLocaleString('fr-CA')} $</td></tr>
           <tr><td>Solde restant (60%)</td><td>${(inv.balance_remaining || 0).toLocaleString('fr-CA')} $</td></tr>`
        : `<tr><td>Montant payé</td><td>${(inv.amount_paid || 0).toLocaleString('fr-CA')} $</td></tr>
           <tr><td>Solde</td><td>${(inv.balance_remaining || 0).toLocaleString('fr-CA')} $</td></tr>`;
      return `
      <div class="invoice-card">
        <div class="invoice-card-header">
          ...
          <span class="status-badge payment-status-${inv.payment_status || 'unpaid'}">${invoiceStatusLabel}</span>
        </div>
...
async function onPaymentOrderSelected(orderId) {
  ...
  const taxesLine = (order.gst_amount > 0 || order.qst_amount > 0)
    ...
    : '';
  const remaining = order.remaining_balance != null ? order.remaining_balance : order.total;
  const depositAlreadyPaid = order.payment_status === 'deposit_paid';
  ...
  summary.innerHTML = `
    ...
    ${fullyPaid
      ? `<div>Montant payé : <strong>${order.total.toLocaleString('fr-CA')} $</strong></div>`
      : depositAlreadyPaid
        ? `<div>Solde restant : <strong>${remaining.toLocaleString('fr-CA', { maximumFractionDigits: 2 })} $</strong></div>`
        : `<div>Dépôt (40%) : ${(order.total * 0.4).toLocaleString('fr-CA', { maximumFractionDigits: 2 })} $</div>`}
  `;

  const fullyPaidNotice = document.getElementById('payment-fully-paid-notice');
  fullyPaidNotice.hidden = !fullyPaid;

  // Bascule les options de montant : Dépôt/Total tant que rien n'est payé,
  // uniquement "Payer le solde" une fois le dépôt reçu — empêche de repayer
  // le montant total et ainsi de facturer le dépôt deux fois.
  document.querySelectorAll('.amount-choice-deposit-full').forEach(el => { el.hidden = depositAlreadyPaid || fullyPaid; });
  document.querySelectorAll('.amount-choice-balance').forEach(el => { el.hidden = !depositAlreadyPaid; });
  if (depositAlreadyPaid) {
    document.querySelectorAll('input[name="interac-amount"][value="balance"], input[name="stripe-amount"][value="balance"]').forEach(el => { el.checked = true; });
  } else {
    document.querySelectorAll('input[name="interac-amount"][value="deposit"], input[name="stripe-amount"][value="deposit"]').forEach(el => { el.checked = true; });
  }

  document.getElementById('payment-methods').hidden = locked || fullyPaid;
  if (locked || fullyPaid) return;
...
const PAYMENT_AMOUNT_LABELS = { deposit: 'Dépôt (40%)', full: 'Montant total', balance: 'Solde restant' };
