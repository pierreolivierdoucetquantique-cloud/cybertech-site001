...
def create_checkout_session(db: Session, order: Order, client: Client, amount_type: PaymentAmountType) -> dict:
    ...
    # Garde-fou : une clé publiable (pk_...) collée par erreur dans
    # STRIPE_SECRET_KEY provoquerait une AuthenticationError Stripe non
    # explicite pour le client — on l'intercepte ici avec un message clair.
    if not settings.STRIPE_SECRET_KEY.startswith("sk_"):
        logger.error("STRIPE_SECRET_KEY mal configurée (ne commence pas par 'sk_').")
        raise HTTPException(status_code=503, detail="Le paiement par carte est mal configuré (clé Stripe invalide). Contactez le support.")

    amount = calculate_payment_amount(db, order, amount_type)
    amount_cents = int(round(amount * 100))

    # Stripe refuse tout montant sous 0,50 $ CAD — on le vérifie avant l'appel
    # réseau pour éviter une InvalidRequestError non gérée (500 générique).
    if amount_cents < 50:
        raise HTTPException(status_code=400, detail="Le montant à payer est trop faible pour être traité par Stripe (minimum 0,50 $ CAD).")
    ...
    try:
        ...
        logger.exception("Clé Stripe invalide ou révoquée lors de la création de la session Checkout.")
        raise HTTPException(status_code=503, detail="Le paiement par carte est temporairement indisponible (configuration Stripe invalide). Contactez le support.")
    except stripe.error.InvalidRequestError as e:
        logger.exception("Requête Stripe invalide lors de la création de la session Checkout.")
        raise HTTPException(status_code=400, detail=f"Impossible de créer la session de paiement : {e.user_message or 'requête invalide.'}")
    except stripe.error.APIConnectionError:
        logger.exception("Impossible de joindre Stripe (problème réseau).")
        raise HTTPException(status_code=502, detail="Impossible de joindre Stripe pour le moment. Veuillez réessayer dans quelques instants.")
    except stripe.error.StripeError as e:
        logger.exception("Erreur Stripe lors de la création de la session Checkout.")
        raise HTTPException(status_code=502, detail=f"Erreur du service de paiement : {e.user_message or 'veuillez réessayer.'}")
